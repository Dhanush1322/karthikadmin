...
function ViewEventGallery() {
    ...
    useEffect(() => {
                    ...
                    for (const category in raw) {
                        if (
                            ...
                            for (const item of raw[category]) {
                                const imageBlob = await fetchImageWithToken(item.img, token);
        ...
        const fetchImageWithToken = async (imgPath, token) => {
            const response = await fetch(
                `https://karthikcreation.ap-1.evennode.com/api/admin/viewEventGalleryFile/${imgPath}`,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                            ...
                            <tr key={item._id}>
                                ...
                                <td>
                                    ...
                                    <button
                                        ...
                                        onClick={() => handleDelete(item._id, item.category)} // Pass both ID and category
                                    >
                                        Delete
                                    </button>
